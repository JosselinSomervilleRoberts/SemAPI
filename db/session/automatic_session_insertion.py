...
if __name__ == "__main__":
    ...
    tomorrow_start_utc = start_utc + 3600 * 24

    # Check if a session already covers the next day
    utc_used = False
    db.cursor.execute("SELECT * FROM public.fr_sessions WHERE utc_start = %s", (tomorrow_start_utc,))
    ...
    if not utc_used:
        ...
        session.SaveToDb(tomorrow_start_utc)
    else:
